...
Modify `parse_query` in `queryParser.js` to recognize `LIKE`` conditions in the `WHERE`` clause.
...
Modify `execute_SELECT_query` in `src/index.js` to filter data using the LIKE operator.
